...
import networkx as nx, matplotlib.pyplot as plt, numpy as np, sklearn.decomposition as skD
...
def train(epochs, model, criterion, optimizer, x, y, edge_index, indicate=False,
          m=mask(None, None),
          plotting=False, scatter_size=30, plotting_freq=5, dim_reduction='pca',
          task='classification'):
    if indicate: print(f'Training the model')
    ...
    for epoch in range(epochs):
        ...
        train_acc_list.append(train_acc)  # each epoch, append accuracy to train_acc_list
...
def visualize_pca(out, color, size=30, epoch=None, loss=None):
    h = skD.PCA(n_components=2).fit_transform(out.detach().cpu().numpy())
    plt.figure(figsize=(7, 7))
    plt.xticks([])
    plt.yticks([])
...
def loo_print(new_output, y_differences, kl, improved_accuracy):
    for node_idx in output.keys():
        print(f'masking {node_idx} improves the accuracy to {improved_accuracy[node_idx]}')
        print(f' - kl score {kl[node_idx]}')
        print(f' - output difference: {y_differences[node_idx]}')
