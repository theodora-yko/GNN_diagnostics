...
def gcn_norm_rw(edge_index, edge_weight=None, num_nodes=None, alpha=0.5, beta=1.,
    ...
    if isinstance(edge_index, SparseTensor):
        ...
        deg_inv_sqrt = deg.pow_(-alpha)
        deg_inv_sqrt.masked_fill_(deg_inv_sqrt == float('inf'), 0)
        L=  edge_index, deg_inv_sqrt[row] * edge_weight * deg_inv_sqrt[col]
...
def gcn_diffusion(edge_index, alpha=0.5, edge_weight=None, num_nodes=None,
             beta=1.0,
...
def gcn_norm_sym(edge_index, alpha=0.5, edge_weight=None, num_nodes=None, beta=1.0,
             add_self_loops=True, dtype=None):
                ...
                if cache is None:
                    if self.gnn_type == "symmetric":
                        ...
                        edge_index, edge_weight = gcn_diffusion(  # yapf: disable
                            edge_index,  self.alpha, edge_weight, x.size(self.node_dim),
                            self.beta, self.add_self_loops)
                    elif self.gnn_type == "symmetrized RW":
                        print("here")
                        edge_index, edge_weight = gcn_norm_sym(  # yapf: disable
                            edge_index,  self.alpha, edge_weight, x.size(self.node_dim),
                            beta=self.beta, add_self_loops=self.add_self_loops)
                    if self.cached:
